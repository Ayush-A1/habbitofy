...
function TodaysHabitsPage() {
  ...
  const toDoHabits = habitsForToday.filter(
    (habit) => !habit.dates.includes(getTodayDate())
  );
  const completedHabits = habitsForToday.filter((habit) =>
    habit.dates.includes(getTodayDate())
  );

  const renderedToDoHabits = toDoHabits.map((habit) => (
    <Habit key={habit.id} data={habit} todaysHabit />
  ));
        ...
        <div className={toDoHabitsClass}>
          {habitsForToday.length > 0 ? (
            renderedToDoHabits.length > 0 ? (
              ...
              <p className="text-center font-bold text-neutral-4">
        ...
        <div className="flex flex-col space-y-4">
          ...
          <div className="flex flex-col space-y-2">
            {renderedCompletedHabits.length > 0 ? (
              ...
              <p className="text-center font-bold text-neutral-4">
                You haven't completed a single habit
              </p>
            )}
          </div>
        </div>
